...
const TenantModal = ({ isOpen, onClose, tenant, unit, onSuccess }: TenantModalProps) => {
    if (!unit) return (
        <Modal
            ...
            content={
                <div className="p-8">
                    <div className="relative bg-red-100 border border-red-200/80 rounded-2xl p-10 shadow-sm">
                        <div className="flex flex-col items-center justify-center space-y-5">
                            <div className="relative">
                                <div className="absolute inset-0 bg-red-400 rounded-full animate-ping opacity-20" />
                                <div className="relative p-5 bg-white rounded-2xl border border-red-200 shadow-sm">
                            ...
                            <div className="text-center space-y-2">
                                ...
                                <p className="text-sm font-medium text-red-600/90 max-w-xs">
                                    Unit information not found. Try <button onClick={() => window.location.reload()} className="underline text-blue-600">refreshing</button> the page.
                                </p>
                            </div>
                        </div>
                    </div>
    ...
    const [first_month_rent, setFirstMonthRent] = useState('');
    ...
    if (!isEdit && unit?.status === 'maintenance') {
        const maintenanceContent = (
            <div className="p-8">
                <div className="relative bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50 border border-amber-200/80 rounded-2xl p-10 shadow-sm">
                    <div className="flex flex-col items-center justify-center space-y-5">
                        <div className="relative">
                            <div className="absolute inset-0 bg-amber-400 rounded-full animate-ping opacity-20" />
                            <div className="relative p-5 bg-white rounded-2xl border border-amber-200 shadow-sm">
                        ...
                        <div className="text-center space-y-2">
                            ...
                            <p className="text-sm font-medium text-amber-600/90 max-w-xs">
    ...
    const inputClass = (field: string) =>
        `w-full px-4 pl-11 py-3.5 rounded-xl border transition-all duration-200 outline-none text-gray-700 placeholder:text-gray-400 text-sm ${
            errors[field]
                ? 'bg-red-50/50 border-red-300 focus:ring-2 focus:ring-red-200 focus:border-red-400'
                : 'border-gray-200 bg-white hover:border-blue-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-100'
        }`;
        ...
        <div className="p-6 sm:p-8 space-y-6 relative">
            ...
            {!isEdit && success && (
                <div className="absolute inset-0 z-20 bg-white/90 backdrop-blur-sm flex items-center justify-center rounded-2xl animate-in fade-in zoom-in duration-300">
                    <div className="text-center p-8 bg-emerald-50 border border-emerald-200 rounded-2xl shadow-lg max-w-md mx-4">
                        <div className="relative inline-block">
                            <CircleCheckBig className="w-14 h-14 text-emerald-500 mx-auto" strokeWidth={1.5} />
                            <div className="absolute inset-0 bg-emerald-400 rounded-full animate-ping opacity-20" />
                        </div>
                        <p className="font-bold text-xl text-emerald-800 mt-4">
                        ...
                        <button
                            onClick={onClose}
                            className="mt-6 px-8 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-xl transition-colors shadow-sm"
                        >
                            Continue
                        </button>
                    </div>
            ...
            <form className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                    ...
                    <div className="space-y-2">
                        <label className="flex items-center gap-1 text-xs font-semibold text-gray-500 uppercase tracking-wider ml-1">
                            Phone Number {!isEdit && <span className="text-red-400">*</span>}
                        </label>
                        <div className="relative">
                    ...
                    <div className="space-y-2">
                        <label className="flex items-center gap-1 text-xs font-semibold text-gray-500 uppercase tracking-wider ml-1">
                            ID Number {!isEdit && <span className="text-red-400">*</span>}
                        </label>
                        <div className="relative">
                            <FileText className="absolute left-4 top-1/2 -translate-y-1/2 w-4.5 h-4.5 text-gray-400" />
                            <input
                                type="text"
