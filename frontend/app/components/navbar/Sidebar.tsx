...
const Sidebar: React.FC<SidebarProps> = ({ appUser }) => {
  const pathname = usePathname();
  const [isOpen, setIsOpen] = useState(true);
  const [isSmallScreen, setIsSmallScreen] = useState(false);
  const [isMediumScreen, setIsMediumScreen] = useState(false);

  const isLandlord = appUser?.user_type === "landlord";
  const isAdmin = appUser?.user_type === "admin";
  const navItems = isLandlord ? LandlordNavItems : isAdmin ? AdminNavItems : TenantNavItems;
