...
import { useCharges } from "@/app/hooks/queries/useUnitDetailQueries";
...
const ChargesTab = ({ unit, tenancyId, onPendingStatusChange }: ChargesTabProps) => {
    const { data: charges = [], isLoading: loading } = useCharges(tenancyId);
    // Reference data, cached globally — this only ever hits the network
    // once per staleTime window, no matter how many unit modals get opened.
    const { data: chargeTypes = [] } = useChargeTypes(!!tenancyId);
    ...
    // Report pending status up to the tab header whenever the charge list
    // (from cache or a fresh fetch) changes.
    useEffect(() => {
        ...
        <div className="space-y-6 animate-in fade-in duration-300">
            ...
            <form onSubmit={handleCreateCharge} className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm space-y-4">
                ...
                {chargeTypes.length === 0 ?
                    <p className="text-xs text-gray-800">No charge types found. Please <Link href="/landlord-portal/settings?tab=charges" className="text-blue-600 hover:underline">add a charge type</Link>.</p>
                    :
                    <button
                        type="submit"
                        disabled={submitting}
