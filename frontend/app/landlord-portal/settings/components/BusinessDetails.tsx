...
import { useUpdateBusinessProfile } from "@/app/hooks/mutations/useSettingsMutations";

const CURRENCIES = ["KES", "USD", "EUR"];

const emptyForm = { company_name: "", email: "", phone: "", address: "", currency: "" };

const BusinessDetails = () => {
  const { data: profile, isLoading } = useBusinessProfile(true);
  ...
  // Populate the editable draft from the server value exactly once, the
  // first time it arrives — not on every render the query data changes —
  // so a background revalidation (or the setQueryData after a save) can
  ...
  useEffect(() => {
    ...
    setCurrentLogo(profile.logo_url ?? null);
  ...
  const hasChanges = () => {
    ...
    return JSON.stringify(form) !== JSON.stringify(baseline) || logoFile !== null;
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 animate-in fade-in duration-500">
      <div className="flex items-center gap-8">
        ...
        <div>
          <h4 className="text-xl font-semibold text-gray-700 uppercase">{form.company_name}</h4>
          <input type="file" accept="image/*" onChange={handleLogoChange} className="text-sm mt-4 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" />
